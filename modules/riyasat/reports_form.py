"""
کارتابل ریاست – بررسی و صدور دستور
...
def get_riyasat_reports_form(user):
...
<style>
    ...
    * {{ margin:0; padding:0; box-sizing:border-box; }}
    ...
    .raiss-header {{
        background:linear-gradient(135deg, #1e3a8a, #3b82f6); color:white; border-radius:16px; padding:25px 30px;
    ...
    .raiss-header h2 {{ font-size:24px; }}
    ...
    @media (max-width:768px) {{
        .raiss-header {{ flex-direction:column; gap:15px; text-align:center; }}
    }}
</style>
</head>
<body>
...
<div class="container fade-in">
    <div class="raiss-header">
        <h2>⚖️ کارتابل ریاست</h2>
        <a href="/module/riyasat" class="back-btn">⬅️ بازگشت</a>
    </div>
    ...
    <div class="card">
        <div class="row" style="margin-bottom:15px;">
            ...
            <select id="status-filter" class="form-select" style="width:180px;" onchange="fetchList(1)">
                ...
                <option value="pending">⏳ منتظر دستور</option>
                <option value="done">✅ دستور صادر شده</option>
            </select>
        </div>
...
<script>
    ...
    async function fetchList(page = 1) {{
        ...
        try {{
            const res = await fetch('/module/riyasat/reports/list?search=' + encodeURIComponent(search) + '&status=' + status + '&page=' + page + '&per_page=15');
            ...
            data.records.forEach(r => {{
                const statusText = r.taid_nazar ? '✅ دستور صادر شده' : '⏳ منتظر دستور';
                const statusClass = r.taid_nazar ? 'status-done' : 'status-pending';
                html += '<tr onclick="loadDetail(' + r.ID_gozaresh + ')">' +
                    ...
                    '<td><button class="btn btn-primary btn-sm" onclick="event.stopPropagation();loadDetail(' + r.ID_gozaresh + ')">' + (r.taid_nazar ? '🔍 مشاهده' : '✏️ بررسی') + '</button></td>' +
                '</tr>';
            }});
    ...
    async function loadDetail(reportId) {{
        try {{
            const res = await fetch('/module/riyasat/reports/get/' + reportId);
            const data = await res.json();
            if (data.success) {{
                const r = data.record;
                ...
                if (r.fanni_opinion) {{
                    html += '<div class="form-group"><label>نظر فنی</label><div class="form-input" style="background:#f9f9f9;">' + r.fanni_opinion + '</div></div>';
                }}

                if (r.taid_nazar) {{
                    html += '<div class="form-group"><label>دستور ریاست</label><textarea class="form-textarea" rows="3" disabled>' + (r.nazar_raiss || '') + '</textarea></div>';
                }} else {{
                    html += '<div class="form-group"><label>دستور / نظر ریاست</label><textarea id="order-text" class="form-textarea" rows="3" placeholder="متن دستور..."></textarea></div>';
                    html += '<button class="btn btn-primary" onclick="saveOrder(' + reportId + ')">✅ ثبت دستور</button>';
                }}
    ...
    async function saveOrder(reportId) {{
        const order = document.getElementById('order-text').value.trim();
        if (!order) {{ showToast('متن دستور الزامی است', 'error'); return; }}
        try {{
            const formData = new FormData();
            formData.append('report_id', reportId);
            formData.append('order_text', order);
            const res = await fetch('/module/riyasat/reports/order', {{ method:'POST', body: formData }});
            const data = await res.json();
            if (data.success) {{
                showToast('✅ دستور ثبت شد', 'success');
                loadDetail(reportId);
...
def get_raiss_reports_list(search, status, page=1, per_page=15):
    ...
    if status == 'pending':
        sql += " AND (nr.taiid_nazar IS NULL OR nr.taiid_nazar = 0)"
        count_sql += " AND (nr.taiid_nazar IS NULL OR nr.taiid_nazar = 0)"
    elif status == 'done':
        sql += " AND nr.taiid_nazar = 1"
        count_sql += " AND nr.taiid_nazar = 1"
...
def save_raiss_order(user, form_data):
    user_id = user.get('UserID', 0)
    report_id = form_data.get('report_id')
    order_text = form_data.get('order_text', '').strip()
    if not order_text:
        return {'success': False, 'message': 'متن دستور الزامی است'}
    today = int(jdatetime.date.today().strftime("%Y%m%d"))
    now = datetime.now()
    try:
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT ID_nazar_raiis FROM tbl_nazar_raiis WHERE kod_gozaresh = %s", (report_id,))
        if cursor.fetchone():
            return {'success': False, 'message': 'دستور قبلاً ثبت شده است'}
        cursor.execute("""
            INSERT INTO tbl_nazar_raiis (kod_gozaresh, nazar_raiss, taiid_nazar, dat_sabt, dat_payan, UserID)
            VALUES (%s, %s, 1, %s, %s, %s)
        """, (report_id, order_text, today, now, user_id))
        conn.commit()
        conn.close()
        return {'success': True, 'message': 'دستور با موفقیت ثبت شد'}
